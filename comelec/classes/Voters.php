...
class Voters
{
    ...
    public function UPDATE_VOTER($student_number, $last_name, $first_name, $middle_name, $section_id, $voters_code, $status, $voter_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Voter was updated successfully.<a href='/spccvotingsystem/comelec/add_voters.php'> <button class='btn btn-outline-primary'>Back</button></a></div>";
        }else{
            echo $stmt->error;
        }
        $stmt->free_result();
