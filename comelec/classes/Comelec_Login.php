...
class Comelec_Login
{
    ...
    public function ComelecLogin() {
        ...
        if($error_msg) {
            ...
            header("location: /spccvotingsystem/comelec/index.php");
            exit();
        }

        $sql = "SELECT * FROM comelec WHERE username = ? AND password = ? LIMIT 1";
        ...
        if($result->num_rows > 0) {
            ...
            $_SESSION['COMELEC_ID']   = $row["id"];
            $_SESSION['COMELEC_NAME'] = $row["name"];
            session_write_close();

            header("location: /spccvotingsystem/comelec/comelec_page.php");
